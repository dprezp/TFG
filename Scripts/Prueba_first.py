...
for dataset_index, dataset_used in enumerate(datasets):
    for attr in protected_attributes[dataset_index]:
        ...
        del clf_first_prune,hist_first_prune
        gc.collect()



#Guardamos el Excel
excel_path = os.path.join("Results", "metrics_Prune_results_{}.xlsx".format(datetime.datetime.now()))
metrics_prune_df.to_excel(excel_path)

prueba_df, invalid = data_tuple
excel_path = os.path.join("Results", "prueba_{}.xlsx".format(datetime.datetime.now()))
prueba_df.to_excel(excel_path)

grafic_df = table_align(data_tuple, first_fairness)

csv_path = os.path.join("Results", "backup_{}.csv".format(datetime.datetime.now()))
grafic_df.to_csv(excel_path, index=False)

excel_path = os.path.join("Results", "grafic_metrics_{}.xlsx".format(datetime.datetime.now()))
