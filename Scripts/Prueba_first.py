...
for i in range(0,10):
    ...
    #Guardamos el Excel
    excel_path = os.path.join("Results", "metrics_Prune_results_{}.xlsx".format(i))
    metrics_prune_df.to_excel(excel_path)

    prueba_df, invalid = data_tuple
    excel_path = os.path.join("Results", "prueba_{}.xlsx".format(i))
    prueba_df.to_excel(excel_path)

    grafic_df = table_align(data_tuple, first_fairness)

    csv_path = os.path.join("Results", "backup_{}.csv".format(i))
    grafic_df.to_csv(excel_path, index=False)

    excel_path = os.path.join("Results", "grafic_metrics_{}.xlsx".format(i))
